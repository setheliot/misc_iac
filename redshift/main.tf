...
resource "aws_redshift_snapshot_copy_grant" "useast2" {
  ...
  snapshot_copy_grant_name = "${local.name}-us-east-2"
  kms_key_id               = aws_kms_key.redshift_us_east_2.arn
...
resource "aws_kms_key" "redshift_us_east_2" {
  provider = aws.us_east_2

  description             = "${local.name} -CMK for Redshift cross-Region snapshot"
  deletion_window_in_days = 7
  enable_key_rotation     = true
